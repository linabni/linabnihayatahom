...
import { useAuth, requireAuth } from 'util/use-auth'
...
const Cont = () => {
  ...
  const { user } = useAuth()
  const { register, handleSubmit, formState } = useForm<ContInputs>({
    criteriaMode: 'all'
  })
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <input value={user?.email ?? ''} type="text" aria-disabled="true" className="mb-2" />
      <Input
...
export default requireAuth(Cont)
